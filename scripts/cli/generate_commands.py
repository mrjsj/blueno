...
from blueno.enums import PrincipalType, WorkspaceRole
...
from blueno import get_fabric_bearer_token
from blueno.core.operations import wait_for_long_running_operation
from blueno.common.remove_none import remove_none
from blueno.common.shorten_dict_values import shorten_dict_values
...
from blueno.common.remove_none import remove_none
{%- for name, subcommand in subcommands.items() %}
from blueno.rest_api import {{ module_name | replace('-', '_') }}_{{ name | replace('-', '_') }}
{%- endfor %}
...
def generate_module():
    ...
    for command in commands:
        ...
        with open(f"src/blueno/cli/commands/{command_name.replace('-', '_')}.py", "w") as f:
        ...
        with open(f"src/blueno/rest_api/{command_name.replace('-', '_')}.py", "w") as f:
            f.write(api_module)

    with open("src/blueno/rest_api/__init__.py", "w") as f:
    ...
    with open("src/blueno/cli/commands/__init__.py", "w") as f:
    ...
    for module, subcommands in sorted(modules):
        with open(f"docs/core/fabric-api/{module.replace('-', '_')}.md", "w") as f:
            ...
            f.write(
                "    Please report any issues to the [GitHub repository](https://github.com/mrjsj/blueno/issues).\n\n"
            )
            f.write(f"::: blueno.rest_api.{module.replace('-', '_')}\n\n")
