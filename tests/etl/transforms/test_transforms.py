...
def test_reorder_columns_by_suffix_no_sorting():
    ...
    actual_df = reorder_columns_by_suffix(
        df, suffix_order=["_key", "_value"], sort_alphabetically_within_group=False
    )
...
def test_reorder_columns_by_prefix_no_sorting():
    ...
    actual_df = reorder_columns_by_prefix(
        df, prefix_order=["dim_", "fact_"], sort_alphabetically_within_group=False
    )
...
def test_deduplicate_single_key():
    df = pl.DataFrame(
        {"id": [1, 2, 2, 3], "value": ["a", "b1", "b2", "c"], "timestamp": [1, 2, 3, 4]}
    )
    ...
    expected_df = pl.DataFrame({"id": [1, 2, 3], "value": ["a", "b2", "c"], "timestamp": [1, 3, 4]})
...
def test_deduplicate_multiple_keys():
    ...
    expected_df = pl.DataFrame(
        {"id": [1, 2], "type": ["A", "B"], "value": ["new", "new"], "timestamp": [2, 4]}
    )
...
def test_normalize_column_names_custom_strategy():
    df = pl.DataFrame({"First Name": ["John", "Jane"], "Last-Name": ["Doe", "Smith"]})
    ...
    expected_df = pl.DataFrame({"FIRST_NAME": ["John", "Jane"], "LAST_NAME": ["Doe", "Smith"]})

    assert_frame_equal(actual_df, expected_df, check_row_order=False)


def test_normalize_column_names_empty_df():
    df = pl.DataFrame({"First Name": [], "Last Name": []})
    ...
    expected_df = pl.DataFrame({"first_name": [], "last_name": []})
